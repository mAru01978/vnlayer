...
export type SpriteAssetConfig = { src: string };

const registry = createAssetRegistry<SpriteAssetConfig>();

const DEFAULT_EXPRESSION = 'normal';

function composeKey(characterName: string, expression: string): string {
  return `${characterName}:${expression}`;
}

export function setSpriteAssets(patch: Record<string, Record<string, SpriteAssetConfig>>): void {
  const flat: Record<string, SpriteAssetConfig> = {};
...
export function setSpriteAssetResolver(
  fn: (characterName: string, expression: string) => SpriteAssetConfig | undefined
): void {
...
export function getSpriteAsset(characterName: string, expression: string | undefined): SpriteAssetConfig | undefined {
  return registry.get(composeKey(characterName, expression ?? DEFAULT_EXPRESSION));
}

export function getAllSpriteAssets(): Record<string, SpriteAssetConfig> {
