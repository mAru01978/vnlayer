...
export type BackgroundSlot = { color?: string; image?: string };

const registry = createAssetRegistry<BackgroundSlot>();

export function setBackgroundSlots(next: Record<string, BackgroundSlot>): void {
  registry.set(next);
}

// テーブル登録の代わりに、命名規則から機械的にパスを組み立てたい場合用
// (例: `(name) => ({ image: \`/assets/backgrounds/${name}.webp\` })`)。
export function setBackgroundResolver(fn: (name: string) => BackgroundSlot | undefined): void {
  registry.setResolver(fn);
}

export function getBackgroundSlot(name: string): BackgroundSlot | undefined {
  return registry.get(name);
}

export function getAllBackgroundSlots(): Record<string, BackgroundSlot> {
