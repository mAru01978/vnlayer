...
export type AnimSpeedConfig = { speeds: Record<string, number> };

const defaultConfig: AnimSpeedConfig = {
  speeds: {
    slow: 0.5,
    normal: 1,
    fast: 2,
...
registerTag<AnimSpeedConfig>({
  key: 'anim_speed',
  defaultConfig,
  run: ({ args, handlers, config }) => {
    const [name, raw] = args;
    const speed = isNumeric(raw) ? Number(raw) : config.speeds[raw];
    if (name && speed !== undefined) handlers.setAnimSpeed(name, speed);
  },
});
