...
export type AnimAssetConfig =
  | { mode: 'sequence'; frames: string[]; fps?: number; source?: ResourceSource; resolveLocal?: (path: string) => Promise<unknown> }
  | { mode: 'single'; src?: string; source?: ResourceSource; resolveLocal?: (path: string) => Promise<unknown> };
...
let resolverFn: ((characterName: string, expression: string, motion: string) => AnimAssetConfig | undefined) | undefined;
...
export function getAnimAsset(
  ...
  if (source === 'local') {
    const resolvedFrames = found.frames.map((frame, i) =>
      resolveUrlCached(`anim:${characterName}:${motion}:seq:${i}`, frame, { source, resolveLocal }, bumpVersion),
    );
    if (resolvedFrames.some((f) => f === undefined)) {
      // まだ全フレーム解決していない: 解決済みの範囲だけでも使えるよう
      // フィルタして返す(空でも呼び出し側でframes.length===0を弾く)。
      return { mode: 'sequence', frames: resolvedFrames.filter((f): f is string => Boolean(f)), fps: found.fps };
    }
    return { mode: 'sequence', frames: resolvedFrames as string[], fps: found.fps };
  }
