...
export function useStoryEngine(
      ...
      const handlers: SceneHandlers = {
        ...
        emitToWeb: (eventName, payload) => {
          ...
          window.dispatchEvent(
            new CustomEvent('vnlayer:emit', { detail: { name: eventName, payload, instanceId } })
          );
        },
  ...
  // 本当に不要になった)。
  const contextSeqRef = useRef<Record<string, number>>({});
  ...
  const contextStoreRef = useRef<Record<string, unknown>>({});
  ...
  const setContextVars = useCallback(
    async (vars: Record<string, unknown>, options?: { notify?: boolean; expose?: boolean }) => {
      ...
      if (options?.notify) {
        wake();
        const withSeq: Record<string, unknown> = { ...vars };
