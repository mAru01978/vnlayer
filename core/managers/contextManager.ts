...
const syncedVarNames = new Map<string, Set<string>>();
// attachStory()がまだ呼ばれていない(Story未生成)段階でsync対象になった
// 変数を、attach後にまとめて登録するための保留リスト。
const pendingSyncVarNames = new Map<string, Set<string>>();
...
const hosts = new Map<string, ContextSyncHost>();
...
function getSyncedSet(atomKey: string): Set<string> {
...
function getPendingSyncSet(atomKey: string): Set<string> {
...
export function attachStory(atomKey: string, host: ContextSyncHost): void {
...
function startObserving(atomKey: string, varName: string): void {
  const synced = getSyncedSet(atomKey);
  if (synced.has(varName)) return;
