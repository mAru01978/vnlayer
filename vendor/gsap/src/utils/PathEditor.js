...
export class PathEditor {
  constructor(target, vars) {
    ...
    this._selection =
      vars._selection ||
      _createSVG("g", this._g, { class: "path-editor-selection path-editor" });
  ...
  init() {
    let pathData = this.path.getAttribute("d"),
      ...
      createAnchors =
        ...
        rawPath.map((s) => s.length).join(",") !==
          this._rawPath.map((s) => s.length).join(","),
      anchorVars = {
        callbackScope: this,
  ...
  _onClickSelectionPath(e) {
    if (this._selection.style.visibility === "hidden") {
      ...
      for (i = 0; i < this._anchors.length; i++) {
        if (
          this._anchors[i].i >= newIndex &&
          this._anchors[i].j === closestData.j
        ) {
          this._anchors[i].i += 6;
        }
      }
