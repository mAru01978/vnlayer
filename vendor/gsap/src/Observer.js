...
let gsap,
  ...
  _onScroll = () =>
    (_normalizer && _normalizer.isPressed) || _scrollers.cache++,
  ...
  _getScrollFunc = (element, { s, sc }) => {
    ...
    func.target = element;
    prev ||
      (func.smooth = gsap.getProperty(element, "scrollBehavior") === "smooth"); // only set it the first time (don't reset every time a scrollFunc is requested because perhaps it happens during a refresh() when it's disabled in ScrollTrigger.
    return func;
  },
...
export class Observer {
  ...
  init(vars) {
    ...
    lineHeight ||
      (lineHeight = parseFloat(_win.getComputedStyle(_body).lineHeight) || 22); // note: browser may report "normal", so default to 22.
    let id,
      onStopDelayedCall,
      dragged,
      moved,
