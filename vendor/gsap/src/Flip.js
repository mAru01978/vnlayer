...
let _id = 1,
  ...
  _closestTenth,
  _getStyleSaver,
  _forEachBatch = (batch, name) =>
    batch.actions.forEach((a) => a.vars[name] && a.vars[name](a)),
  _batchLookup = {},
  ...
  _listToArray = (list) =>
    typeof list === "string" ? list.split(" ").join("").split(",") : list, // removes extra spaces contaminating the names, returns an Array.
  ...
  _getEl = (target) =>
    _toArray(target)[0] || console.warn("Element not found:", target),
  ...
  _orderByDOMDepth = (comps, invert, isElStates) => {
    comps.forEach(
      (comp) =>
        (comp.d = _getDOMDepth(isElStates ? comp.element : comp.t, invert)),
    );
  ...
  _makeAbsolute = (elState, fallbackNode, ignoreBatch) => {
    ...
    if (_batch && ignoreBatch !== 1) {
      ...
      _batch._final.push(
        () => (elState.cache.uncache = 1) && _applyInlineStyles(elState),
      );
      return element;
    }
  ...
  _elementsFromElementStates = (elStates) =>
    elStates.map((elState) => elState.element),
  ...
  _fit = (fromState, toState, scale, applyProps, fitChild, vars) => {
    let { element, cache, parent, x, y } = fromState,
      ...
      styles =
        vars &&
        _getStyleSaver &&
        _getStyleSaver(element, "transform,width,height"), // requires at least 3.11.5
    ...
    if (simple || !parent) {
      ...
      scaleY =
        Math.sqrt(matrix.c ** 2 + matrix.d ** 2) * Math.cos(skewX * _DEG2RAD);
      if (fitChild) {
    ...
    if (scale) {
      scaleX *=
        width === dimensionState.width || !dimensionState.width
          ? 1
          : width / dimensionState.width; // note if widths are both 0, we should make scaleX 1 - some elements have box-sizing that incorporates padding, etc. and we don't want it to collapse in that case.
      scaleY *=
  ...
  _parseState = (targetsOrState, vars) =>
    targetsOrState instanceof FlipState
      ? targetsOrState
...
class FlipState {
  ...
  update(soft) {
    this.elementStates = this.targets.map(
      (el) => new ElementState(el, this.props, this.simple),
    );
    _createLookup(this);
    this.interrupt(soft);
  ...
  fit(state, scale, nested) {
    let elStatesInOrder = _orderByDOMDepth(
  ...
  compare(state) {
    let l1 = state.idLookup,
      ...
      placeIfDoesNotExist = (s1, s2, el) =>
        targets.indexOf(el) < 0 && place(s1, s2, el),
      s1,
      s2,
      p,
      el,
    ...
    for (p in l1) {
      ...
      if (s2Alt) {
        c2 =
          s2.isVisible || (!s2Alt.isVisible && el === s2.element) ? s2 : s2Alt;
        c1 =
          s1Alt &&
          !s1.isVisible &&
          !s1Alt.isVisible &&
...
class ElementState {
  ...
  update(props, simple) {
    ...
    self.width = bbox
      ? bbox.width
      : _closestTenth(getProp("width", "px"), 0.04); // round up to the closest 0.1 so that text doesn't wrap.
    self.height = bbox
      ? bbox.height
...
class FlipBatch {
  ...
  add(config) {
    ...
    result = new FlipAction(
      typeof config === "function" ? { animate: config } : config,
      this,
    );
  ...
  setState() {
    this.actions.forEach(
      (c) => (c.targets = c.vars.setState && c.vars.setState(c)),
    );
    return this;
  }
...
export class Flip {
  ...
  static fit(fromEl, toEl, vars) {
    let v = vars ? _copy(vars, _fitReserved) : {},
      { absolute, scale, getVars, props, runBackwards, onComplete, simple } =
        vars || v,
  ...
  static killFlipsOf(targets, complete) {
    (targets instanceof FlipState
      ? targets.targets
      : _toArray(targets)
    ).forEach((t) => t && _killFlip(t._flip, complete !== false ? 1 : 2));
  }
