...
let gsap,
  ...
  _subdivideSegmentQty = (segment, quantity) => {
    ...
    for (; i < l; i += 6) {
      distances.push(
        (segment[i] - segment[i + 6]) ** 2 +
          (segment[i + 1] - segment[i + 7]) ** 2,
      );
    }
    while (quantity--) {
...
export const MorphSVGPlugin = {
  ...
  init(target, value, tween, index, targets) {
    ...
    smooth === true || smooth === "auto"
      ? (smooth = {})
      : typeof smooth === "number" && (smooth = { points: smooth });
      ...
      if (p === "d" || this._prop) {
        ...
        if (smooth) {
          ...
          _smoothRawPath(
            ...
            smooth.redraw === false ? smooth : { ...smooth, points: start },
          );
        }
        if (
          !precompiled &&
