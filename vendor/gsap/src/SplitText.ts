...
let gsap: any,
  ...
  _splitProp: symbol | string =
    typeof Symbol === "function" ? Symbol() : "_split",
  _coreInitted: boolean, // set to true when the GSAP core is registered
  _initIfNecessary = () =>
    _coreInitted || SplitText.register((window as any).gsap),
  _charSegmenter: any =
    typeof Intl !== "undefined" && "Segmenter" in Intl
      ? new (Intl as any).Segmenter()
      : 0, // not all older browsers support Intl.Segmenter
  _toArray = (r: string | NodeList | Node | Node[]): Node[] =>
    !r
      ? []
      : typeof r === "string"
        ? _toArray(document.querySelectorAll(r))
        : "length" in r
          ? Array.from(r).reduce((acc: Node[], cur: Node | string) => {
              typeof cur === "string"
                ? acc.push(..._toArray(cur))
                : acc.push(cur);
              return acc;
            }, [])
          : [r],
  _elements = (targets: SplitTextTarget): HTMLElement[] =>
    _toArray(targets).filter((e) => e && e.nodeType === 1) as HTMLElement[], // don't use instanceof HTMLElement because it's not always reliable (e.g. in an iframe). See https://github.com/greensock/GSAP/issues/640
  _emptyArray: string[] = [],
  _context: ContextFunction = function () {},
  _defaultContext: { add: (f: Function) => any } = { add: (f) => f() },
  _spacesRegEx: RegExp = /\s+/g,
  _emojiSafeRegEx: RegExp =
  ...
  _findNextValidBounds = (
    allBounds: BoundingRect[],
    startIndex: number,
  ): BoundingRect => {
    // searches through the allBounds Array to find the next non-empty bounds after the startIndex (non-inclusive). Returns the empty bounds if no valid bounds are found.
    while (
  ...
  _stretchToFitSpecialChars = (
    ...
    if (specialCharsRegEx) {
      let charsFound: Set<string> = new Set(
          collection.join("").match(specialCharsRegEx) || _emptyArray,
        ),
        i: number = collection.length,
        slots: number,
        word: string,
        char: string,
        combined: string;
  ...
  _disallowInline = (element: HTMLElement): unknown =>
    window.getComputedStyle(element).display === "inline" &&
    (element.style.display = "inline-block"),
  _insertNodeBefore = (
    newChild: Node | string,
    parent: Element,
    existingChild: Node,
  ): Node =>
    parent.insertBefore(
  ...
  _splitWordsAndCharsRecursively = (
    element: Element,
    config: SplitTextConfig,
    wordWrapper: WrapFunction,
    charWrapper: WrapFunction | null,
    prepForCharsOnly: boolean,
    deepSlice: boolean,
    ignore: Element[] | false,
    ...
    let nodes: Node[] = Array.from(element.childNodes),
      ...
      curSubNode: Node | null,
      tempSubNode: Node,
      curTextContent: string,
      wordText: string,
      lastWordText: string,
      k: number;
    ...
    if (typeof wordDelimiter === "object") {
      wordDelimSplitter =
        (wordDelimiter as WordDelimiterConfig).delimiter ||
        (wordDelimiter as string | RegExp);
      wordDelimString =
        (wordDelimiter as WordDelimiterConfig).replaceWith || "";
    } else {
    ...
    for (; i < nodes.length; i++) {
      ...
      if (curNode.nodeType === 3) {
        ...
        for (j = 1; j <= words.length; j++) {
          ...
          if (!reduceWhiteSpace && wordText === "") {
            ...
            if (
              ...
              j === 1 &&
              !startsWithSpace &&
              wordsCollection.indexOf(
                (ignoredPreviousSibling as Element).parentNode as HTMLElement,
              ) > -1
            ) {
              curWordEl = wordsCollection[wordsCollection.length - 1];
              curWordEl.appendChild(
                document.createTextNode(charWrapper ? "" : wordText),
              ); // if we're splitting characters, we'll add them one-by-one below
            } else {
              ...
              ignoredPreviousSibling &&
                ...
                curWordEl.insertBefore(
                  ignoredPreviousSibling as Element,
            ...
            if (deepSlice && isNested) {
              ...
              ); // remember that we've got to accommodate the word delimiter in the substring.
              ...
              if (
                ...
                (element.parentNode as Element).insertBefore(
                  clonedNode,
                  element,
                );
        ...
        if (ignore && ignore.indexOf(curNode as Element) > -1) {
          // if the current node is in the ignore array, then we need to ignore it and move it inside the end of the last word (but only if the previous sibling is a word).
          wordsCollection.indexOf(curNode.previousSibling as HTMLElement) >
            -1 &&
...
export class SplitText {
  ...
  constructor(elements: SplitTextTarget, config: SplitTextConfig) {
    ...
    this.elements.forEach((el: HTMLElement) => {
      // avoid re-splitting the same element multiple times
      config.overwrite !== false &&
        (el as any)[_splitProp]?._data.orig
          .filter(({ element }: SplitTextOriginal) => element === el)
    ...
    this._data = {
      ...
      obs:
        ...
        new ResizeObserver(() => {
          ...
          timerId = setTimeout(checkWidths, 200) as unknown as number;
        }),
    };
  ...
  split(config: SplitTextConfig) {
    (this._ctx || _defaultContext).add(() => {
      ...
      let {
        ...
        onlySplitCharacters: boolean =
          splitCharacters && !splitWords && !splitLines,
        specialCharsRegEx: RegExp | undefined =
          specialChars &&
          ("push" in specialChars
            ? new RegExp("(?:" + specialChars.join("|") + ")", "gu")
            : specialChars),
        finalCharSplitRegEx: RegExp = specialCharsRegEx
          ? new RegExp(
        ...
        ignore: HTMLElement[] | false =
          !!config.ignore && _elements(config.ignore),
      ...
      if (splitCharacters || splitWords || splitLines) {
        this.elements.forEach((element, index) => {
          ...
          if (splitLines) {
            let nodes: Node[] = _toArray(element.childNodes),
              ...
              allBounds: BoundingRect[] = nodes.map((n) =>
                n.nodeType === 1
                  ? (n as Element).getBoundingClientRect()
                  : _emptyBounds,
              ), // do all these measurements at once so that we don't trigger layout thrashing
            ...
            for (i = 0; i < nodes.length; i++) {
              ...
              if (curNode.nodeType === 1) {
                if (curNode.nodeName === "BR") {
                  // remove any <br> tags because breaking up by lines already handles that.
            ...
            for (i = 0; i < words.length; i++) {
              ...
              if (
                ...
                if (smartWrap && !splitLines) {
                  // tried inserting String.fromCharCode(8288) between each character to prevent words from wrapping strangely, but it doesn't work. Also tried adding <wbr> tags between each character, but it doesn't work either.
        ...
        mask &&
          this[mask] &&
          this.masks.push(
            ...(this as any)[mask as string].map((el: HTMLElement) => {
      ...
      if (
        (onSplitResult = onSplit && onSplit(this)) &&
        onSplitResult.totalTime
      ) {
        this._data.anim = animTime
  ...
  revert() {
    if (this.isSplit) {
      ...
      this.isSplit = false;
      if (anim) {
        // if the user returned an animation in the onSplit, we record the totalTime() here and revert() it and then we'll the totalTime() of the newly returned onSplit animation. This allows things to be seamless
