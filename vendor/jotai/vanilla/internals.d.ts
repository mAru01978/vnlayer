...
declare function hasInitialValue<T extends AnyAtom>(
  atom: T,
): atom is T &
  (T extends Atom<infer Value>
    ? {
        init: Value;
      }
    : never);
type ActuallyWritableAtom<T extends AnyAtom> =
  T extends WritableAtom<infer V, infer A, infer R>
    ...
    : T extends Atom<infer V>
      ...
      : never;
declare function isActuallyWritableAtom<T extends AnyAtom>(
  atom: T,
): atom is ActuallyWritableAtom<T>;
declare function isAtomStateInitialized(
  atomState: AtomState<AnyValue>,
): boolean;
...
declare function addPendingPromiseToDependency(
  atom: AnyAtom,
  promise: PromiseLike<AnyValue>,
  dependencyAtomState: AtomState,
): void;
declare function getMountedOrPendingDependents(
  atom: AnyAtom,
  atomState: AtomState,
  mountedMap: MountedMap,
): Iterable<AnyAtom>;
...
declare function initializeStoreHooks(
  storeHooks: StoreHooks,
): Required<StoreHooks>;
declare function getBuildingBlocks(store: Store): Readonly<BuildingBlocks>;
declare function buildStore(
  ...partialBuildingBlocks: Partial<BuildingBlocks>
): Store;
export {
