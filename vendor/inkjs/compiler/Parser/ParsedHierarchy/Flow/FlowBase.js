...
class FlowBase extends Object_1.ParsedObject {
  ...
  get name() {
    var _a;
    return (
      ((_a = this.identifier) === null || _a === void 0 ? void 0 : _a.name) ||
      null
    );
  }
  constructor(
    ...
    this.SplitWeaveAndSubFlowContent = (contentObjs, isRootStory) => {
      ...
      for (const obj of contentObjs) {
        ...
        if (subFlow) {
          ...
          if (
            (_a = subFlow.identifier) === null || _a === void 0
              ? void 0
              : _a.name
          ) {
            this._subFlowsByName.set(
              (_b = subFlow.identifier) === null || _b === void 0
                ? void 0
                : _b.name,
              subFlow,
            );
      ...
      if (isRootStory) {
        weaveObjs.push(
          new Gather_1.Gather(null, 1),
          new Divert_1.Divert(new Path_1.Path(Identifier_1.Identifier.Done())),
        );
      }
      const finalContent = [];
    ...
    this.ResolveVariableWithName = (varName, fromNode) => {
      ...
      const ownerFlow =
        fromNode === null
          ? this
          : (0, ClosestFlowBase_1.ClosestFlowBase)(fromNode);
      if (ownerFlow) {
        // Argument
        if (ownerFlow.args !== null) {
          for (const arg of ownerFlow.args) {
            if (
              ((_a = arg.identifier) === null || _a === void 0
                ? void 0
                : _a.name) === varName
            ) {
              result.found = true;
    ...
    this.GenerateRuntimeObject = () => {
      ...
      if (
        this.flowLevel !== FlowLevel_1.FlowLevel.Story &&
        !this.isFunction &&
        this._rootWeave !== null &&
        foundReturn === null
      ) {
    ...
    this.GenerateArgumentVariableAssignments = (container) => {
      ...
      for (let ii = this.args.length - 1; ii >= 0; --ii) {
        const paramName =
          ((_a = this.args[ii].identifier) === null || _a === void 0
            ? void 0
            : _a.name) || null;
        const assign = new VariableAssignment_1.VariableAssignment(
          paramName,
          true,
        );
    ...
    this.DeepSearchForAnyLevelContent = (name) => {
      const weaveResultSelf = this.ContentWithNameAtLevel(
        name,
        FlowLevel_1.FlowLevel.WeavePoint,
        false,
      );
      if (weaveResultSelf) {
    ...
    this.CheckForDisallowedFunctionFlowControl = () => {
      ...
      for (const divert of allDiverts) {
        if (
          ...
          !(divert.parent instanceof DivertTarget_1.DivertTarget)
        ) {
          this.Error(
    ...
    this.WarningInTermination = (terminatingObject) => {
      ...
      if (
        terminatingObject.parent === this._rootWeave &&
      ...
      const terminatingDivert = (0, TypeAssertion_1.asOrNull)(
        terminatingObject,
        Divert_1.Divert,
      );
        ...
        for (let jj = ii + 1; jj < this.args.length; jj += 1) {
          if (
            ((_a = this.args[ii].identifier) === null || _a === void 0
              ? void 0
              : _a.name) ==
            ((_b = this.args[jj].identifier) === null || _b === void 0
              ? void 0
              : _b.name)
          ) {
            this.Error(
