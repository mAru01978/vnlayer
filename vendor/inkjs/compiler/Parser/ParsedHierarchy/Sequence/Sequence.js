...
class Sequence extends Object_1.ParsedObject {
  constructor(elementContentLists, sequenceType) {
    ...
    this.GenerateRuntimeObject = () => {
      ...
      const stopping =
        (this.sequenceType & SequenceType_1.SequenceType.Stopping) > 0;
      const shuffle =
        (this.sequenceType & SequenceType_1.SequenceType.Shuffle) > 0;
      ...
      if (stopping || once) {
        ...
        container.AddContent(
          NativeFunctionCall_1.NativeFunctionCall.CallWithName("MIN"),
        );
      } else if (cycle) {
        // - Cycle: take (read count % num elements)
        container.AddContent(
          new Value_1.IntValue(this.sequenceElements.length),
        );
        container.AddContent(
          NativeFunctionCall_1.NativeFunctionCall.CallWithName("%"),
        );
      }
      // Shuffle
      if (shuffle) {
        ...
        container.AddContent(
          ControlCommand_1.ControlCommand.SequenceShuffleIndex(),
        );
        if (once || stopping) {
    ...
    this.AddDivertToResolve = (divert, targetContent) => {
      this._sequenceDivertsToResolve.push(
        new SequenceDivertToResolve_1.SequenceDivertToResolve(
          divert,
          targetContent,
        ),
      );
