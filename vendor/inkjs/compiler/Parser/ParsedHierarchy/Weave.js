  ...
  constructor(cont, indentIndex = -1) {
    ...
    this.ResolveWeavePointNaming = () => {
      ...
      const namedWeavePoints = [
        ...this.FindAll(Gather_1.Gather)(
          (w) => !(w.name === null || w.name === undefined),
        ),
        ...this.FindAll(Choice_1.Choice)(
          (w) => !(w.name === null || w.name === undefined),
        ),
      ];
      this._namedWeavePoints = new Map();
      for (const weavePoint of namedWeavePoints) {
        ...
        const existingWeavePoint = this.namedWeavePoints.get(
          ((_a = weavePoint.identifier) === null || _a === void 0
            ? void 0
            : _a.name) || "",
        );
        if (existingWeavePoint) {
          const typeName =
            existingWeavePoint instanceof Gather_1.Gather ? "gather" : "choice";
          const existingObj = existingWeavePoint;
          this.Error(
            `A ${typeName} with the same label name '${weavePoint.name}' already exists in this context on line ${
        ...
        if (
          (_b = weavePoint.identifier) === null || _b === void 0
            ? void 0
            : _b.name
        ) {
          this.namedWeavePoints.set(
            (_c = weavePoint.identifier) === null || _c === void 0
              ? void 0
              : _c.name,
            weavePoint,
          );
    ...
    this.AddRuntimeForGather = (gather) => {
      ...
      for (const looseEndWeavePoint of this.looseEnds) {
        ...
        this.gatherPointsToResolve.push(
          new GatherPointToResolve_1.GatherPointToResolve(
            divert,
            gatherContainer,
          ),
        );
    ...
    this.AddRuntimeForWeavePoint = (weavePoint) => {
      ...
      if (this.WeavePointHasLooseEnd(weavePoint)) {
        ...
        const looseChoice = (0, TypeAssertion_1.asOrNull)(
          weavePoint,
          Choice_1.Choice,
        );
        if (looseChoice) {
    ...
    this.PassLooseEndsToAncestors = () => {
      ...
      for (
        ...
        if (
          ancestor instanceof Sequence_1.Sequence ||
          ancestor instanceof Conditional_1.Conditional
        ) {
          nested = true;
        }
      }
      ...
      for (let ii = this.looseEnds.length - 1; ii >= 0; ii -= 1) {
        ...
        if (nested) {
          ...
          //    if there is one
          if (
            looseEnd instanceof Choice_1.Choice &&
            closestInnerWeaveAncestor !== null
          ) {
          ...
          if (
            closestInnerWeaveAncestor === null ||
            closestInnerWeaveAncestor === void 0
              ? void 0
              : closestInnerWeaveAncestor.hasOwnProperty("ReceiveLooseEnd")
          ) {
            closestInnerWeaveAncestor.ReceiveLooseEnd(looseEnd);
          }
          received = true;
    ...
    this.IsGlobalDeclaration = (obj) => {
      const varAss = (0, TypeAssertion_1.asOrNull)(
        obj,
      ...
      const constDecl = (0, TypeAssertion_1.asOrNull)(
        obj,
        ConstantDeclaration_1.ConstantDeclaration,
      );
      if (constDecl) {
        return true;
      }
    ...
    this.ContentThatFollowsWeavePoint = (weavePoint) => {
      ...
      for (
        let ii = weavePointIdx + 1;
        ii < parentWeave.content.length;
        ii += 1
      ) {
        ...
        if (
          laterObj instanceof Choice_1.Choice ||
          laterObj instanceof Gather_1.Gather
        ) {
          break;
        }
    ...
    this.ValidateTermination = (badTerminationHandler) => {
      ...
      if (
        this.lastParsedSignificantObject instanceof
        AuthorWarning_1.AuthorWarning
      ) {
        return;
      }
      ...
      if (hasLooseEnds) {
        ...
        for (const obj of this.content) {
          if (
            obj instanceof Choice_1.Choice ||
            obj instanceof Divert_1.Divert
          ) {
            return;
          }
        }
    ...
    this.BadNestedTerminationHandler = (terminatingObj) => {
      ...
      for (
        ...
        if (
          ...
          conditional = (0, TypeAssertion_1.asOrNull)(
            ancestor,
    ...
    this.ValidateFlowOfObjectsTerminates = (
      ...
      for (const flowObj of objFlow) {
        const divert = flowObj.Find(Divert_1.Divert)(
          (d) =>
            !d.isThread &&
            !d.isTunnel &&
            !d.isFunctionCall &&
    ...
    this.WeavePointHasLooseEnd = (weavePoint) => {
      ...
      for (let ii = weavePoint.content.length - 1; ii >= 0; --ii) {
        let innerDivert = (0, TypeAssertion_1.asOrNull)(
          weavePoint.content[ii],
          Divert_1.Divert,
        );
        if (innerDivert) {
          const willReturn =
