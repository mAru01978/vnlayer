  ...
  constructor(functionName, args) {
    ...
    this.GenerateIntoContainer = (container) => {
      ...
      if (this.isChoiceCount) {
        ...
        const divertTarget = (0, TypeAssertion_1.asOrNull)(
          this.args[0],
          DivertTarget_1.DivertTarget,
        );
        const variableDivertTarget = (0, TypeAssertion_1.asOrNull)(
          this.args[0],
        ...
        for (let ii = 0; ii < this.args.length; ii += 1) {
          const num = (0, TypeAssertion_1.asOrNull)(
            this.args[ii],
            NumberExpression_1.NumberExpression,
          );
          if (num && !num.isInt()) {
        ...
        NativeFunctionCall_1.NativeFunctionCall.CallExistsWithName(this.name)
      ) {
        const nativeCall = NativeFunctionCall_1.NativeFunctionCall.CallWithName(
          this.name,
        );
        ...
        container.AddContent(
          NativeFunctionCall_1.NativeFunctionCall.CallWithName(this.name),
        );
      ...
      if (this.shouldPopReturnedValue) {
        container.AddContent(
          ControlCommand_1.ControlCommand.PopEvaluatedValue(),
        );
      }
    };
    this.toString = () => {
    ...
    this._proxyDivert = new Divert_1.Divert(
      new Path_1.Path(functionName),
      args,
    );
