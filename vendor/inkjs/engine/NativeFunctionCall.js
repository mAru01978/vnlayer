...
class NativeFunctionCall extends Object_1.InkObject {
  ...
  set name(value) {
    ...
    if (!this._isPrototype) {
      if (NativeFunctionCall._nativeFunctions === null)
        (0, NullException_1.throwNullException)(
  ...
  CallType(parametersOfSingleType) {
    let param1 = (0, TypeAssertion_1.asOrThrows)(
      parametersOfSingleType[0],
      Value_1.Value,
    );
    ...
    if (paramCount == 2 || paramCount == 1) {
      ...
      if (paramCount == 2) {
        let param2 = (0, TypeAssertion_1.asOrThrows)(
          parametersOfSingleType[1],
          Value_1.Value,
        );
        let val2 = param2;
        ...
        if (val1.value === null)
          return (0, NullException_1.throwNullException)(
            "NativeFunctionCall.Call UnaryOp value",
          );
  ...
  CallBinaryListOperation(parameters) {
    ...
    if (
      (this.name == "&&" || this.name == "||") &&
      (v1.valueType != Value_1.ValueType.List ||
        v2.valueType != Value_1.ValueType.List)
    ) {
      if (this._operationFuncs === null)
        return (0, NullException_1.throwNullException)(
          "NativeFunctionCall._operationFuncs",
      ...
      let result = (0, TypeAssertion_1.asBooleanOrThrows)(
  ...
  CallListIncrementOperation(listIntParams) {
    let listVal = (0, TypeAssertion_1.asOrThrows)(
      listIntParams[0],
      Value_1.ListValue,
    );
    let intVal = (0, TypeAssertion_1.asOrThrows)(
      listIntParams[1],
      Value_1.IntValue,
    );
  ...
  CoerceValuesToSingleType(parametersIn) {
    ...
    let parametersOut = [];
    if (
      Value_1.ValueType[valType] == Value_1.ValueType[Value_1.ValueType.List]
    ) {
      for (let inkObjectVal of parametersIn) {
        ...
        if (val.valueType == Value_1.ValueType.List) {
          ...
          specialCaseList = (0, TypeAssertion_1.asOrThrows)(
            specialCaseList,
            Value_1.ListValue,
          );
          ...
          let item = list.TryGetItemWithValue(
            intVal,
            InkList_1.InkListItem.Null,
          );
          if (item.exists) {
  ...
  static GenerateNativeFunctionsIfNecessary() {
    if (this._nativeFunctions == null) {
      ...
      this.AddOpToNativeFunc(
        this.Equal,
        2,
        Value_1.ValueType.DivertTarget,
        divertTargetsEqual,
      );
      this.AddOpToNativeFunc(
        this.NotEquals,
