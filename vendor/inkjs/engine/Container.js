...
class Container extends Object_1.InkObject {
  ...
  get namedOnlyContent() {
    ...
    for (let [key, value] of this.namedContent) {
      let inkObject = (0, TypeAssertion_1.asOrThrows)(
        value,
        Object_1.InkObject,
      );
  ...
  TryAddNamedContent(contentObj) {
    let namedContentObj = (0, TypeAssertion_1.asINamedContentOrNull)(
      contentObj,
    );
  ...
  AddToNamedContentOnly(namedContentObj) {
    Debug_1.Debug.AssertType(
      namedContentObj,
      Object_1.InkObject,
      "Can only add Runtime.Objects to a Runtime.Container",
    );
    let runtimeObj = (0, TypeAssertion_1.asOrThrows)(
      namedContentObj,
      Object_1.InkObject,
    );
  ...
  ContentWithPathComponent(component) {
    if (component.isIndex) {
      ...
      let foundContent = (0, TryGetResult_1.tryGetValueFromMap)(
        this.namedContent,
        component.name,
        null,
      );
    ...
    for (let [key, value] of this.namedContent) {
      if (
        this.content.indexOf(
          (0, TypeAssertion_1.asOrThrows)(value, Object_1.InkObject),
        ) >= 0
      ) {
        continue;
      } else {
