  ...
  const play = async () => {
    ...
    do {
      ...
      for (let i = 0; i < story.currentChoices.length; ++i) {
        ...
        if (
          story.currentChoices[i].tags !== null &&
          story.currentChoices[i].tags!.length > 0
        ) {
          process.stdout.write(
            " # tags: " + story.currentChoices[i].tags!.join(", "),
          );
        }
        process.stdout.write("\n");
      }
      ...
      do {
        ...
        if (answer.startsWith("->")) {
          ...
          const saveName =
            answer.slice(4).trim() ||
            `quicksave-${new Date().toISOString().slice(0, 19).replace(/[-:]/g, "")}.json`; //formatted as YYYYMMDDTHHMMSS
          lastSave = saveName;
