#!/usr/bin/env node
// 使い方: node scripts/new-character.js mika 40 55
//   data/characterSlots.json に { "mika": { "originX": 40, "originY": 55 } } を追記する
// 既に同名キャラがいる場合は座標を上書きする(確認メッセージを出す)

const fs = require('fs');
const path = require('path');

const [, , name, originXArg, originYArg] = process.argv;

if (!name || originXArg === undefined || originYArg === undefined) {
  console.error('使い方: node scripts/new-character.js <キャラ名> <originX> <originY>');
...
if (Number.isNaN(originX) || Number.isNaN(originY)) {
  console.error('originX / originY は数値で指定してください(0〜100のパーセント想定)');
  process.exit(1);
}

const jsonPath = path.join('data', 'characterSlots.json');
...
if (isUpdate) {
  console.log(`updated: "${name}" の座標を (${originX}, ${originY}) に上書きしました`);
} else {
  console.log(`added: "${name}" を (${originX}, ${originY}) で追加しました`);
}
console.log('\n完了。あとはInk側で # c:' + name + ':表情 / # m:' + name + ' を書くだけです。');
