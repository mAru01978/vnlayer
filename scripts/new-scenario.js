...
if (!scenarioName) {
  console.error('使い方: node VNLayer/scripts/new-scenario.js シナリオ名 [dataディレクトリのパス]');
  process.exit(1);
}
...
console.log('');
console.log('次にやること:');
console.log(`  1. ${path.join(displayDir, 'knots', 'start.ink')} を実際の内容に書き換える`);
console.log('  2. npm run compile-story でコンパイル (パスを変えている場合は対象に合わせて実行)');
