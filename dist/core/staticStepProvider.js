...
async function loadStoryJson(clip, dataBaseUrl, loaderOptions) {
    try {
        return await loadJson(`${clip}/story.json`, {
...
async function createStoryHandle(clip, dataBaseUrl, loaderOptions, key) {
    ...
    contextManager.attachStory(key, {
        observeVariable: (varName, onChange) => {
            story.ObserveVariable(varName, (_name, newValue) => {
                onChange(newValue);
            });
        },
    });
...
export function createStaticStepProvider(options = {}) {
    ...
    return {
        ...
        async getSaveData(clip, atomKey) {
            const key = cacheKey(clip, atomKey);
            return withLock(key, async () => {
        ...
        async restore(clip, save, atomKey) {
            ...
            return withLock(key, async () => {
                ...
                try {
                    ...
                    reportError(new StoryRuntimeError("failed to restore ink state from save data, starting fresh instead", { cause: e }));
                    return runAndCache(handle);
                }
