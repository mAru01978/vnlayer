...
function useAssetsVersion(): void {
...
function shortestRotationTo(prevDeg: number, rawTargetDeg: number): number {
...
function CharacterSprite({ name, state, slot, isFocused, hasSpeaker, onClick, atomKey }: CharacterSpriteProps) {
  useAssetsVersion();
  const rootRef = useRef<HTMLDivElement>(null);
  const arrowRef = useRef<HTMLDivElement>(null);
  const imgRef = useRef<HTMLImageElement>(null);
  const videoRef = useRef<HTMLVideoElement>(null);
  const positionTlRef = useRef<gsap.core.Timeline | null>(null);
  const gazeTlRef = useRef<gsap.core.Timeline | null>(null);
  const sequenceTlRef = useRef<gsap.core.Timeline | null>(null);
...
const hasRealAsset = Boolean(
  animAsset || (spriteSrc && !imageFailed)
);
useEffect(() => {
...
const allowMock =
  ...
  useGSAP(
    () => {
      ...
      if (!animAsset || animAsset.mode !== 'sequence' || !imgRef.current || !state.motion) return;
      ...
      tl.to(proxy, {
        ...
        onUpdate: () => {
          if (imgRef.current) imgRef.current.src = frames[Math.round(proxy.frame)];
        },
      });
  ...
  useGSAP(
    () => {
      ...
      if (!animAsset || animAsset.mode !== 'single' || !animAsset.src || !video || !state.motion) return;
  ...
  return (
    <>
      ...
      >
        ...
        {animAsset?.mode === 'single' && animAsset.src && (
          <video
            ref={videoRef}
            src={animAsset.src}
            muted
...
{!animAsset && spriteSrc && !imageFailed && (
  ...
  />
      ...
      {/* 視線矢印。位置(left/top)はキャラ本体と同じslotの値を初期表示だけに
          使い、キャラが動けば矢印も一緒に追従して見えるよう、こちらは
          位置だけCSSのままにしてある(rotateだけがGSAP管理の対象)。 */}
      {gazeAngle !== null && (
        <div
          ref={arrowRef}
          style={{
...
function MessageBubble({ speaker, content, slot, revealedCount, visible, onClick, fontFamily, fontSizePx, offsetPx, atomKey }: MessageBubbleProps) {
  const rootRef = useRef<HTMLDivElement>(null);
  const positionTlRef = useRef<gsap.core.Timeline | null>(null);
  const hasPositionedRef = useRef(false);
...
function ChoiceButton({ text, onClick, disabled, fontFamily, fontSizePx, index }: ChoiceButtonProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
...
function FlashOverlay({ color, durationMs, atomKey }: FlashOverlayProps) {
  const ref = useRef<HTMLDivElement>(null);
  const tlRef = useRef<gsap.core.Timeline | null>(null);
...
export const Renderer: StageRenderer = {
  Background,
  CharacterSprite,
  MessageBubble,
  NarratorCaption,
