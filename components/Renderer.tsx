...
function resolveBgVisual(bg: string, scope?: string): BgVisual {
...
function Background({ bg, atomKey, zIndex, instanceId }: BackgroundProps) {
  useAssetsVersion();
        ...
        {!hasRealAsset && (
          <>
            ...
            <div style={{ fontSize: 10, opacity: 0.8 }}>
              ...
              {state.animLoop ? " 🔁" : ""}
              {state.animReverse ? " ⏪" : ""}
              {state.animSpeed !== undefined && state.animSpeed !== 1
                ? ` x${state.animSpeed}`
                : ""}
            </div>
          </>
...
function FlashOverlay({ color, durationMs, atomKey }: FlashOverlayProps) {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(
      ref.current,
      { opacity: 1 },
      { opacity: 0, duration: durationMs / 1000, ease: "power1.out" },
    );
    return () => {
      tl.kill();
      timelineManager.unregister(atomKey, tl);
