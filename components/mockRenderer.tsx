...
type BgVisual = { kind: 'image'; src: string } | { kind: 'color'; color: string };
...
function resolveBgVisual(bg: string): BgVisual {
  ...
  if (slot?.image) return { kind: 'image', src: slot.image };
  if (slot?.color) return { kind: 'color', color: slot.color };
...
function CharacterSprite({ name, state, slot, isFocused, hasSpeaker, onClick, atomKey }: CharacterSpriteProps) {
  ...
  useGSAP(
    () => {
      ...
      if (!animAsset || animAsset.mode !== 'single' || !video || !state.motion) return;
